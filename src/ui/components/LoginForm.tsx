...
import { handleSubmit } from "@/app/[channel]/login/actions";
...
export function LoginForm() {
	...
	const submitHandler = async (formData: FormData) => {
		...
		if (result instanceof Error) {
			...
			router.replace("/default-channel");
			setIsLoading(false);
		}
	};
		...
		<>
			<div className="flex h-full w-full flex-col items-center justify-center gap-12 md:flex-row md:justify-around md:gap-0">
				<div className="flex flex-col justify-center gap-2">
					<h1 className="text-2xl md:text-3xl">Member Log In</h1>
				...
				<form className="flex flex-col  justify-center gap-4" action={submitHandler}>
					...
					<div className="flex justify-center">
						<Button label="Masuk" type="submit" variant="primary" loading={isLoading} />
					</div>
				</form>
